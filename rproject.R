...
dataset <- read.csv("C:/Users/Sheetal/RepData_PeerAssessment1/activity/activity.csv")

#total number of missing data
sum(is.na(dataset$steps))/dim(dataset)[[1]]
...
library(ggplot2)
Q2<-data.frame(tapply(dataset$steps,dataset$date,sum,na.rm=TRUE))
Q2$date<-rownames(Q2)
rownames(Q2)<-NULL
...
library(dplyr)
Q3<-data.frame(round(tapply(dataset$steps,dataset$date,mean,na.rm=TRUE),2))
Q3$date<-rownames(Q3)
rownames(Q3)<-NULL
names(Q3)[[1]]<-"Mean Steps"
temp<-dataset%>%select(date,steps) %>% group_by(date) %>% summarise(median(steps))
...
ggplot(Q4,aes(x=Q4$date,y=Q4$`Mean Steps`))+geom_bar(stat="identity")+scale_x_date()+ylab("Mean Steps Each Date")+xlab("Date")+ggtitle("Time Series of Average Steps")
png("plot4.png")

#5.The 5-minute interval that, on average, contains the maximum number of steps
data$interval<-factor(data$interval)
Q5<-aggregate(data=dataset,steps~date+interval,FUN="mean")
